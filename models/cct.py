...
import torch
from models.compact_transformers.utils.helpers import pe_check, fc_check
from models.compact_transformers.utils.tokenizer import Tokenizer
from models.compact_transformers.utils.transformers import TransformerClassifier
from models.resnet import HierarchicalSoftmax
...
#
# class HierarchicalCCT(nn.Module):
#
#     def __init__(self,
#                  img_size=224,
#                  embedding_dim=768,
...
#                  num_classes=1000,
#                  positional_embedding='learnable',
#                  ntokens_per_class=10,
#                  *args, **kwargs):
#         super().__init__()
#         self.backbone = CCT(img_size=img_size,
#                             embedding_dim=embedding_dim,
...
class HierarchicalCCT(nn.Module):
    ...
    def __init__(self, backbone, num_classes, ntokens_per_class, **kwargs):
        super(HierarchicalCCT, self).__init__()
        self.backbone = backbone
        self.backbone.classifier.fc = nn.Linear(self.backbone.classifier.fc.in_features, 256)
...
#
# def _hcct(arch, pretrained, progress,
#           num_layers, num_heads, mlp_ratio, embedding_dim, ntokens_per_class,
#           kernel_size=3, stride=None, padding=None,
#           positional_embedding='learnable',
#           *args, **kwargs):
#     stride = stride if stride is not None else max(1, (kernel_size // 2) - 1)
#     padding = padding if padding is not None else max(1, (kernel_size // 2))
#     model = HierarchicalCCT(num_layers=num_layers,
#                             num_heads=num_heads,
#                             mlp_ratio=mlp_ratio,
#                             embedding_dim=embedding_dim,
#                             kernel_size=kernel_size,
#                             stride=stride,
#                             padding=padding,
#                             ntokens_per_class=ntokens_per_class,
#                             *args, **kwargs)
#     if pretrained:
#         if arch in model_urls:
#             state_dict = load_state_dict_from_url(model_urls[arch],
#                                                   progress=progress)
#             if positional_embedding == 'learnable':
#                 state_dict = pe_check(model.backbone, state_dict)
#             elif positional_embedding == 'sine':
#                 state_dict['classifier.positional_emb'] = model.backbone.state_dict()['classifier.positional_emb']
#             state_dict = fc_check(model.backbone, state_dict)
#             model.backbone.load_state_dict(state_dict)
#         else:
#             raise RuntimeError(f'Variant {arch} does not yet have pretrained weights.')
#     model.backbone.classifier.fc = nn.Linear(embedding_dim, 128)
#     return model
...
# def hcct_14(arch, pretrained, progress, ntokens_per_class, *args, **kwargs):
#     backbone = cct_14('cct_14_7x2_384_fl', pretrained, progress,
#                       kernel_size=7, n_conv_layers=2,
#                       img_size=224, positional_embedding="learnable",
#                       num_classes=1000,
#                       *args, **kwargs)
#     return _hcct(arch, pretrained, progress, ntokens_per_class=ntokens_per_class, num_layers=14, num_heads=6, mlp_ratio=3,
#                  embedding_dim=384,
#                  *args, **kwargs)
...
@register
def hcct_14_7x2_224(pretrained=False, progress=False, ntokens_per_class=10,
                    img_size=224, positional_embedding='learnable', num_classes=1000,
                    *args, **kwargs):
    backbone = cct_14_7x2_224(pretrained=pretrained, img_size=img_size, positional_embedding=positional_embedding, num_classes=1000)
    return HierarchicalCCT(backbone=backbone, num_classes=num_classes, ntokens_per_class=ntokens_per_class, **kwargs)
